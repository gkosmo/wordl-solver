...
class TestWordlSolverDetailed < Test::Unit::TestCase
  def test_find_possible_words_detailed
    ...
    # Check if the words are 5 letters long
    puts "Are all words 5 letters long? #{LETTER_WORDS.all? { |word| word.length == 5 }}"
    if !LETTER_WORDS.all? { |word| word.length == 5 }
      puts "Words that are not 5 letters long:"
      LETTER_WORDS.each_with_index do |word, index|
        puts "#{index}: #{word} (length: #{word.length})" if word.length != 5
      end
    end
